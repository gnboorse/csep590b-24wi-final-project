...
#define QUERY_LENGTH 10000 // TODO: change me?
...
int main(int argc, char **argv) {
    ...
    wbGenerateParams_t paramsQuery;
    paramsQuery.raw.rows   = 512;
    paramsQuery.raw.cols   = 2000;
    paramsQuery.raw.minVal = 58;
    paramsQuery.raw.maxVal = 120;
    paramsQuery.raw.type   = wbType_float;

    char *randomQueryFileName = wbPath_join(wbDirectory_current(), "query-string", "query.raw");
    wbDataset_generate(randomQueryFileName, wbExportKind_raw, paramsQuery);
    wbTime_stop(Generic, "Generating random query");
    wbLog(TRACE, "Creaated 512 random querys of length 2000 at query-string/query.raw");
    ...
    char *inputFileName = wbPath_join(wbDirectory_current(), "reference-string", "random-number-string.raw");
    char *outputFileName = wbPath_join(wbDirectory_current(), "reference-string", "normalized-reference-string.raw");
    ...
    hostNormalizerInput = (float *)wbImport(inputFileName, &inputLength);

    int numBlockSums = QUERY_LENGTH / (BLOCK_SIZE << 1);
    if (QUERY_LENGTH % (BLOCK_SIZE << 1)) {
        numBlockSums++;
    }
    
    int inputSizeBytes = QUERY_LENGTH * sizeof(float);
    ...
    float mean = hostOutputSums[0] / QUERY_LENGTH;
    float variance = hostOutputSquaredSums[0];
    variance  = variance/QUERY_LENGTH- mean*mean;
    ...
    // initialize grid/block dimensions for the kernel that calculates sums
    int numNormalizeThreadsPerBlock = QUERY_LENGTH / (BLOCK_SIZE * ITEMS_PER_THREAD);
    if (QUERY_LENGTH % (BLOCK_SIZE * ITEMS_PER_THREAD)) {
    ...
    wbExport(outputFileName, (wbReal_t *)hostNormalizerOutput, 1, QUERY_LENGTH);

    wbLog(TRACE, "normalized ", QUERY_LENGTH, " values");
